...
volatile  uint32_t time[41],count,bits[40];
volatile  uint32_t trise, tfall,have_rise_timestamp;
...
/**converts the pulse width to bits**/
void get_bits()
{
	for(int i=1;i<41;i++)
	{
...
/** converts the bits to 8 bits data**/
...

{
    ...
    GPIOA->MODER |= 1U;
	GPIOA->OTYPER |= 1U;
	tim5_init_1us();// delay generation timer initialisation
	GPIOA->BSRR = (1U << 16);
	delay_ms(1);// HIGH 30us (release while still output)
    ...
    while(count>=41);//wait till the 40 bits are captured
    get_bits();// converts the pulse width to bits
	/**humidity and temperature is calculated**/
